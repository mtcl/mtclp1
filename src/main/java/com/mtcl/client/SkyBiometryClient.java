...
import com.mtcl.rpi2.led.LedBlink;
...
public class SkyBiometryClient {
	...
	public static void main(String[] args) {
		try {
			...
			String api_key = "bb6a75cdda9542ac93fce337c539ac11";
			String api_secret = "060b3895eb534a62bbbbdba93cc2175c";
			...
			// FormDataBodyPart facePicBodyPart = new FormDataBodyPart("files",
			// facePic,MediaType.APPLICATION_OCTET_STREAM_TYPE);
			FormDataBodyPart facePicBodyPart = new FormDataBodyPart("urls",
					"http://image.naldzgraphics.net/2012/09/22-SPsil1.jpg");
			...
			if (response.getStatus() != 200) {
				...
				try {
					// if eye parameter is returned, its open.
					eyesOpen = root.getPhotos().get(0).getTags().get(0).getAttributes().getEyes().getValue();

				} catch (NullPointerException e) {
					// if eye parameter is not passed, eyes are closed.
					eyesOpen = "Closed";
				}

				System.out.println(eyesOpen);

				LedBlink ledBlink = new LedBlink();

				String color = "green";
				if (eyesOpen.equalsIgnoreCase("open")) {
					color = "green";
				} else {
					color = "red";
				}
				...
				try {
					...
					System.out.println("Run the program on PI2, this wont run on an architecture without GPIO PINS");
				}
